export default <any> {
    car: {
        translation:'ДТП',
        damage: 'hp',
        text: 'По дорозі додому ваш працівник, який займає посаду "_", потрапив у ДТП. Переходячи дорогу у встановленому місці на працівника був скоєний наїзд через порушення водієм ПДД. Працівник отримав черепно-мозкову травму та численні синці, садна',
        treatment: {
            tablets: {
                translation: 'пігулки',
                healthImprovement: 0.1,
                price: 50
            },
            sleep: {
                translation: 'сон',
                healthImprovement: 0.2,
                price: 60
            },
            hotBath: {
                translation: 'гарячі ванни',
                healthImprovement: 0.3,
                price: 70
            },
            vitamins: {
                translation: 'вітаміни',
                healthImprovement: 0.4,
                price: 80
            }
        }
    },
    electricity: {
        translation:'Ураження струмом',
        damage: 'hp',
        text: 'Під час робочого процесу ваш працівник, який займає посаду \'_\', отримав тілесні ушкодження, спричинені ураженням електричним струмом. Причиною виявилася несправність дротів, які були підключені до робочого комп\'ютера',
        treatment: {
            tablets: {
                translation: 'пігулки',
                healthImprovement: 0.1,
                price: 50
            },
            sleep: {
                translation: 'сон',
                healthImprovement: 0.2,
                price: 60
            },
            hotBath: {
                translation: 'гарячі ванни',
                healthImprovement: 0.3,
                price: 70
            },
            vitamins: {
                translation: 'вітаміни',
                healthImprovement: 0.4,
                price: 80
            }
        }
    },
    hotWater: {
        translation:'Опік окропом',
        damage: 'hp',
        text: 'Під час обідньої перерви ваш працівник, який займає посаду "_", випадково пролив на себе окріп, що спричинило опіки на уражених ділянках.',
        treatment: {
            tablets: {
                translation: 'пігулки',
                healthImprovement: 0.1,
                price: 50
            },
            sleep: {
                translation: 'сон',
                healthImprovement: 0.2,
                price: 60
            },
            hotBath: {
                translation: 'гарячі ванни',
                healthImprovement: 0.3,
                price: 70
            },
            vitamins: {
                translation: 'вітаміни',
                healthImprovement: 0.4,
                price: 80
            }
        }
    },
    nervous: {
        translation:'Отримання нервового зриву',
        damage: 'hp',
        text: 'Через проблеми в сім\'ї, недостатню заробітну плату та погану погоду ваш робітник, який займає посаду "_", отримав нервовий зрив. Через це працівник зовсім не може зосередитися на даних йому завданнях, що сильно знижує ефективність роботи',
        treatment: {
            tablets: {
                translation: 'пігулки',
                healthImprovement: 0.1,
                price: 50
            },
            sleep: {
                translation: 'сон',
                healthImprovement: 0.2,
                price: 60
            },
            hotBath: {
                translation: 'гарячі ванни',
                healthImprovement: 0.3,
                price: 70
            },
            vitamins: {
                translation: 'вітаміни',
                healthImprovement: 0.4,
                price: 80
            }
        }
    },
    crane: {
        translation:'Удар будівельним краном',
        damage: 'hp',
        text: '_ строительний кран',
        treatment: {
            tablets: {
                translation: 'пігулки',
                healthImprovement: 0.1,
                price: 50
            },
            sleep: {
                translation: 'сон',
                healthImprovement: 0.2,
                price: 60
            },
            hotBath: {
                translation: 'гарячі ванни',
                healthImprovement: 0.3,
                price: 70
            },
            vitamins: {
                translation: 'вітаміни',
                healthImprovement: 0.4,
                price: 80
            }
        }
    },
    shawermaIntoxication: {
        translation:'Отруєння шавермою',
        damage: 'hp',
        text: '_ отравление шаурмой',
        treatment: {
            tablets: {
                translation: 'пігулки',
                healthImprovement: 0.1,
                price: 50
            },
            sleep: {
                translation: 'сон',
                healthImprovement: 0.2,
                price: 60
            },
            hotBath: {
                translation: 'гарячі ванни',
                healthImprovement: 0.3,
                price: 70
            },
            vitamins: {
                translation: 'вітаміни',
                healthImprovement: 0.4,
                price: 80
            }
        }
    },
    steamBurn: {
        translation:'Опік парою',
        damage: 'hp',
        text: '_ опік парою',
        treatment: {
            tablets: {
                translation: 'пігулки',
                healthImprovement: 0.1,
                price: 50
            },
            sleep: {
                translation: 'сон',
                healthImprovement: 0.2,
                price: 60
            },
            hotBath: {
                translation: 'гарячі ванни',
                healthImprovement: 0.3,
                price: 70
            },
            vitamins: {
                translation: 'вітаміни',
                healthImprovement: 0.4,
                price: 80
            }
        }
    }
}